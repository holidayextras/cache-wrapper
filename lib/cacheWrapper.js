...
cacheWrapper.initialise = function( serverConfig, cachePolicies ) {
  ...
  // if serverConfig is empty use an in-memory version of the cache
  var catboxType = _.isEmpty( serverConfig ) ? catboxMemory : catboxRedis;
...
cacheWrapper._delete = function( options ) {
  ...
  if ( policies[options.segment] ) {
    ...
    _.each( options.keys, function( key ) {
      ...
      policies[options.segment].drop( key, function( error ) {
        deferred.resolve();
      } );
    } );
  } else {
